...
import { Package, Plus, LogOut, LayoutGrid, Mail, Upload, Zap } from "lucide-react";
  ...
  useEffect(() => {
    fetch("/api/admin/check")
      .then((r) => {
        ...
        else { setAuthed(false); router.replace("/admin-login"); }
      })
      .catch(() => { setAuthed(false); router.replace("/admin-login"); });
  }, []);
      ...
      <nav className="bg-white dark:bg-[#1A1816] border-b border-[#E8E2DC] dark:border-white/10 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 lg:px-8 flex items-center justify-between h-14">
          <div className="flex items-center gap-6">
            <Link href="/admin" className="flex items-center gap-2 text-[#3D3730] dark:text-[#D4C8B8] font-bold">
              <Package className="w-5 h-5 text-[#B8A080]" />
              <span className="text-sm">后台管理</span>
            </Link>
            <Link href="/admin" className="text-xs font-medium text-[#6B6058] dark:text-white/40 hover:text-[#3D3730] dark:hover:text-white transition-colors">
              <LayoutGrid className="w-3.5 h-3.5 inline mr-1" />产品列表
            </Link>
            <Link href="/admin/products/new" className="text-xs font-medium text-[#6B6058] dark:text-white/40 hover:text-[#3D3730] dark:hover:text-white transition-colors">
          ...
          <button onClick={logout} className="flex items-center gap-1 text-xs text-[#9B8E7E] dark:text-white/40 hover:text-red-500 transition-colors">
