...
export default function AdminDashboard() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter((p) =>
        p.name.toLowerCase().includes(q) || (p.nameZh && p.nameZh.includes(search)) ||
        (p.model && p.model.toLowerCase().includes(q)) || p.id.includes(q)
      );
    }
    // Sort
    if (sort === "name") list = [...list].sort((a, b) => (a.nameZh || a.name).localeCompare(b.nameZh || b.name, "zh"));
    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex gap-1.5 flex-wrap">
          ...
          {catTabs.map((cat) => (
            <button key={cat} onClick={() => { setCatFilter(catFilter === cat ? "" : cat); setPage(0); }}
            ...
            <div className="p-2.5">
              ...
              <div className="flex items-center justify-between mt-1">
                ...
                <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button onClick={() => handleDelete(p.id, p.nameZh || p.name)}
